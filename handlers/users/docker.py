...
async def get_archive(message: types.Message):
    if message.from_user.id in [user.id_user for user in Session.query(User)] or \
            message.from_user.id not in [user.id_user for user in Session.query(TrialAccess)] or \
            Session.query(TrialAccess).filter(TrialAccess.id_user == message.from_user.id)[0].is_trial:
        ...
        if name[-4:] != '.zip':
            ...
            if not key:
                ...
                state = dp.current_state(user=BaseStates.all_states[0])
